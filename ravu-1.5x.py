...
class RAVU_15x(userhook.UserHook):
    """
    1.5x upscaling variant of RAVU-Lite, compute shader only
    """

    def __init__(self,
                 profile=Profile.luma,
                 weights_file=None,
                 lut_name="ravu_15x_lut",
                 **args):
        super().__init__(**args)
            ...
            for j in range(self.quant_strength):
                for k in range(self.quant_coherence):
                    ...
                    while len(w2) % 4 > 0:
                        w2.append(0.0)
                    assert len(w1) == n * n * 4
                    assert len(w1) + len(w2) == self.lut_width * 4
                    weights.extend(w1)
                    weights.extend(w2)
...
""" % (math.pi, math.pi, eps, eps))
    ...
    def apply_convolution_kernel(self, samples_list):
        ...
        GLSL("$sample_type res2 = $sample_zero;")
        GLSL("vec4 w0, w1, w2;")
        ...
        for i in range(n * n // 2):
            ...
            if i % 4 == 0:
                coord_x = (float(n * n + i // 4) + 0.5) / float(self.lut_width)
                GLSL("w2 = texture(%s, vec2(%s, coord_y));" % (self.lut_name, coord_x))
            j = n * n - 1 - i
            if i < j:
                GLSL("res0 += %s + %s;" %
                ...
                GLSL("res2 += (%s + %s) * w2[%d];" % (samples_list[i], samples_list[j], i % 4))
            elif i == j:
                GLSL("res0 += %s;" %
                    self.outer_product(samples_list[i], "w0"))
                GLSL("res1 += %s;" %
                    self.outer_product(samples_list[i], "w1"))
                GLSL("res2 += %s * w2[%d];" % (samples_list[i], i % 4))
    ...
    def generate(self, block_size):
        ...
        self.set_description("RAVU-1.5x (%s, r%d)" % (self.profile.name, self.radius))

        block_width, block_height = block_size

        self.set_skippable(1.5, 1.5)
        self.add_cond_even("HOOKED.w")
        self.add_cond_even("HOOKED.h")
        self.set_transform(1.5, 1.5, 0.0, 0.0)
        ...
        array_size = block_width * 2 + n - 2, block_height * 2 + n - 2
...
void hook() {""")
        ...
        # load all samples
        GLSL("ivec2 group_base = ivec2(gl_WorkGroupID) * ivec2(gl_WorkGroupSize) * 2;")
        GLSL("int local_pos = (int(gl_LocalInvocationID.x) * %d + int(gl_LocalInvocationID.y)) * 2;" % array_size[1])
...
for (int id = int(gl_LocalInvocationIndex); id < %d; id += int(gl_WorkGroupSize.x * gl_WorkGroupSize.y)) {""" % (array_size[0] * array_size[1]))
        ...
        samples_list = []
        for dx in range(1 - self.radius, self.radius + 1):
            for dy in range(1 - self.radius, self.radius + 1):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="generate RAVU-1.5x user shader for mpv")
    parser.add_argument(
        '-t',
        '--target',
        nargs=1,
