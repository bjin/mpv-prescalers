    ...
    def apply_convolution_kernel(self, samples_list):
        ...
        for i in range(self.lut_width):
            ...
            if i < j:
                ...
                if use_ar:
                    GLSL("cg4 = vec4(1.0 + %s, 2.0 - %s, 1.0 + %s, 2.0 - %s);" % (samples_list[i], samples_list[i], samples_list[j], samples_list[j]))
                    GLSL("cg4_1 = cg4;")
                    GLSL("cg4 *= cg4;" * 5)
                ...
                if use_ar:
                    GLSL("vec2 cg2 = vec2(1.0 + %s, 2.0 - %s);" % (samples_list[i], samples_list[i]))
                    GLSL("vec2 cg2_1 = cg2;")
                    GLSL("cg2 *= cg2;" * 5)
                    GLSL("hi += cg2.x * wg;")
                    GLSL("lo += cg2.y * wg;")
